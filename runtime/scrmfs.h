...
#include <stdint.h>
#include <mpi.h>
#include "scrmfs-log-format.h"

/* Environment variable to override CP_JOBID */
#define CP_JOBID_OVERRIDE "SCRMFS_JOBID"

/* Environment variable to override __CP_LOG_PATH */
#define CP_LOG_PATH_OVERRIDE "SCRMFS_LOGPATH"

/* Environment variable to override __CP_LOG_PATH */
#define CP_LOG_HINTS_OVERRIDE "SCRMFS_LOGHINTS"

/* Environment variable to override __CP_MEM_ALIGNMENT */
#define CP_MEM_ALIGNMENT_OVERRIDE "SCRMFS_MEMALIGN"
...
/* maximum number of access sizes and stride sizes that scrmfs will track
 ...
 */
#define CP_COMP_BUF_SIZE ((CP_MAX_FILES * sizeof(struct scrmfs_file))/2)
...
struct scrmfs_file_runtime
{
    struct scrmfs_file* log_file;
    struct scrmfs_file_runtime* name_next;
    struct scrmfs_file_runtime* name_prev;
    void* access_root;
    int access_count;
    void* stride_root;
    int stride_count;
...
#define SCRMFS_FILE_HANDLE_MAX (sizeof(MPI_File))
/* This struct is used to track a reference to a file by file 
 * descriptor, MPI file handle, ncdf id, etc.
 */
struct scrmfs_file_ref
{
    struct scrmfs_file_runtime* file;
    char handle[SCRMFS_FILE_HANDLE_MAX];
    int handle_sz;
    enum scrmfs_handle_type handle_type;
    struct scrmfs_file_ref* next;
    struct scrmfs_file_ref* prev;
};

/* in memory structure to keep up with job level data */
struct scrmfs_job_runtime
{
    struct scrmfs_job log_job;
    char exe[CP_EXE_LEN+1];
    struct scrmfs_file file_array[CP_MAX_FILES];
    struct scrmfs_file_runtime file_runtime_array[CP_MAX_FILES];
    char comp_buf[CP_COMP_BUF_SIZE];
    int flags;
    int file_count;
    struct scrmfs_file_runtime* name_table[CP_HASH_SIZE];
    struct scrmfs_file_ref* handle_table[CP_HASH_SIZE];
};
...
extern struct scrmfs_job_runtime* scrmfs_global_job;
...
uint64_t scrmfs_hash(const register unsigned char *k, register uint64_t length, register uint64_t level);
