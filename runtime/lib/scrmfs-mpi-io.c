...
#define CP_DATATYPE_INC(__file, __datatype) do {\
    ...
    ret = SCRMFS_MPI_CALL(PMPI_Type_get_envelope)(__datatype, &num_integers, \
...
#define CP_RECORD_MPI_WRITE(__ret, __fh, __count, __datatype, __counter, __tm1, __tm2) do { \
    ...
    file = scrmfs_file_by_fh(__fh); \
    if(!file) break; \
    SCRMFS_MPI_CALL(PMPI_Type_size)(__datatype, &size);  \
    size = size * __count; \
    SCRMFS_MPI_CALL(PMPI_Type_extent)(__datatype, &extent); \
...
static void cp_log_construct_indices(struct scrmfs_job_runtime* final_job,
    int rank, int* inout_count, int* lengths, void** pointers, char*
    trailing_data);
static int cp_log_write(struct scrmfs_job_runtime* final_job, int rank, 
    char* logfile_name, int count, int* lengths, void** pointers, double start_log_time);
static void cp_log_record_hints(struct scrmfs_job_runtime* final_job, int rank);
static int cp_log_reduction(struct scrmfs_job_runtime* final_job, int rank, 
    char* logfile_name, MPI_Offset* next_offset);
static void scrmfs_file_reduce(void* infile_v, 
    void* inoutfile_v, int *len, 
    MPI_Datatype *datatype);
static int cp_log_compress(struct scrmfs_job_runtime* final_job,
...
static char*  scrmfs_get_exe_and_mounts(struct scrmfs_job_runtime* final_job);
static int scrmfs_file_variance(
    struct scrmfs_file *infile_array,
    struct scrmfs_file *outfile_array,
    int count, int rank);
...
static struct scrmfs_file_runtime* scrmfs_file_by_fh(MPI_File fh);
static void scrmfs_file_close_fh(MPI_File fh);
static struct scrmfs_file_runtime* scrmfs_file_by_name_setfh(const char* name, MPI_File fh);
...
void scrmfs_mpi_initialize(int *argc, char ***argv)
{
    ...
    SCRMFS_MPI_CALL(PMPI_Comm_size)(MPI_COMM_WORLD, &nprocs);
    SCRMFS_MPI_CALL(PMPI_Comm_rank)(MPI_COMM_WORLD, &rank);
    ...
    else
    {
        ...
        scrmfs_initialize(0, NULL, nprocs, rank);
    }
...
void scrmfs_shutdown(int timing_flag)
{
    int rank;
    char* logfile_name;
    struct scrmfs_job_runtime* final_job;
     ...
     * write it out
     */
    final_job = scrmfs_global_job;
    scrmfs_global_job = NULL;
    CP_UNLOCK();

    start_log_time = SCRMFS_MPI_CALL(PMPI_Wtime)();

    /* figure out which access sizes to log */
    scrmfs_walk_file_accesses(final_job);
     ...
     */
    trailing_data = scrmfs_get_exe_and_mounts(final_job);
    ...
    bcst1=SCRMFS_MPI_CALL(PMPI_Wtime)();
    SCRMFS_MPI_CALL(PMPI_Bcast)(mnt_id_array_root,
        CP_MAX_MNTS*sizeof(int64_t), MPI_BYTE, 0, MPI_COMM_WORLD);
    SCRMFS_MPI_CALL(PMPI_Bcast)(mnt_hash_array_root,
        CP_MAX_MNTS*sizeof(uint64_t), MPI_BYTE, 0, MPI_COMM_WORLD);
    bcst2=SCRMFS_MPI_CALL(PMPI_Wtime)();
    ...
    if(rank == 0)
    {
        ...
        logmod = scrmfs_hash((void*)hname,strlen(hname),0);

        /* see if scrmfs was configured using the --with-logpath-by-env
        ...
        if(logpath_override)
        {
            ret = snprintf(logfile_name, PATH_MAX, 
                "%s/%s_%s_id%d_%d-%d-%d-%" PRIu64 ".scrmfs_partial",
                logpath_override, 
        ...
        {
            ret = snprintf(logfile_name, PATH_MAX, 
                "%s/%d/%d/%d/%s_%s_id%d_%d-%d-%d-%" PRIu64 ".scrmfs_partial",
    ...
    bcst3=SCRMFS_MPI_CALL(PMPI_Wtime)();
    SCRMFS_MPI_CALL(PMPI_Bcast)(logfile_name, PATH_MAX, MPI_CHAR, 0,
        MPI_COMM_WORLD);
    ...
    if(timing_flag)
        red1 = SCRMFS_MPI_CALL(PMPI_Wtime)();
    local_ret = cp_log_reduction(final_job, rank, logfile_name, 
        &next_offset);
    if(timing_flag)
        red2 = SCRMFS_MPI_CALL(PMPI_Wtime)();
    SCRMFS_MPI_CALL(PMPI_Allreduce)(&local_ret, &all_ret, 1, MPI_INT, MPI_LOR, 
        MPI_COMM_WORLD);
    ...
    if(all_ret == 0)
    {
        /* compress data */
        if(timing_flag)
            gz1 = SCRMFS_MPI_CALL(PMPI_Wtime)();
        local_ret = cp_log_compress(final_job, rank, &index_count, 
            lengths, pointers);
        if(timing_flag)
            gz2 = SCRMFS_MPI_CALL(PMPI_Wtime)();
        SCRMFS_MPI_CALL(PMPI_Allreduce)(&local_ret, &all_ret, 1,
    ...
    if(rank == 0)
    {
        if(all_ret != 0)
        {
            fprintf(stderr, "scrmfs library warning: unable to write log file %s\n", logfile_name);
        ...
        {
            /* rename from *.scrmfs_partial to *-<logwritetime>.scrmfs.gz,
            ...
            if(new_logfile_name)
            {
                ...
                end_log_time = SCRMFS_MPI_CALL(PMPI_Wtime)();
                strcat(new_logfile_name, logfile_name);
                mod_index = strstr(new_logfile_name, ".scrmfs_partial");
                sprintf(mod_index, "_%d.scrmfs.gz", (int)(end_log_time-start_log_time+1));
    ...
    if(timing_flag)
    {
        ...
        tm_end = SCRMFS_MPI_CALL(PMPI_Wtime)();
        ...
        SCRMFS_MPI_CALL(PMPI_Allreduce)(&red_tm, &red_slowest, 1,
            MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
        SCRMFS_MPI_CALL(PMPI_Allreduce)(&gz_tm, &gz_slowest, 1,
            MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
        SCRMFS_MPI_CALL(PMPI_Allreduce)(&write_tm, &write_slowest, 1,
            MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
        SCRMFS_MPI_CALL(PMPI_Allreduce)(&all_tm, &all_slowest, 1,
            MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
        SCRMFS_MPI_CALL(PMPI_Allreduce)(&bcst_tm, &bcst_slowest, 1,
            MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
...
int MPI_File_open(MPI_Comm comm, char *filename, int amode, MPI_Info info, MPI_File *fh) 
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_open)(comm, filename, amode, info, fh);
    tm2 = scrmfs_wtime();
    ...
    if(ret == MPI_SUCCESS)
    {
        ...
        file = scrmfs_file_by_name_setfh(filename, (*fh));
        if(file)
        {
            ...
            if(CP_F_VALUE(file, CP_F_OPEN_TIMESTAMP) == 0)
                ...
                SCRMFS_MPI_CALL(PMPI_Wtime)());
            SCRMFS_MPI_CALL(PMPI_Comm_size)(comm, &comm_size);
            if(comm_size == 1)
            {
...
int MPI_File_close(MPI_File *fh) 
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_close)(fh);
    tm2 = scrmfs_wtime();

    CP_LOCK();
    file = scrmfs_file_by_fh(tmp_fh);
    if(file)
    {
        CP_F_SET(file, CP_F_CLOSE_TIMESTAMP, SCRMFS_MPI_CALL(PMPI_Wtime)());
        CP_F_INC_NO_OVERLAP(file, tm1, tm2, file->last_mpi_meta_end, CP_F_MPI_META_TIME);
        scrmfs_file_close_fh(tmp_fh);
    }
    CP_UNLOCK();
...
int MPI_File_sync(MPI_File fh)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_sync)(fh);
    tm2 = scrmfs_wtime();
    if(ret == MPI_SUCCESS)
    {
        CP_LOCK();
        file = scrmfs_file_by_fh(fh);
        if(file)
        {
    ...
    MPI_Datatype filetype, char *datarep, MPI_Info info)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_set_view)(fh, disp, etype,
    ...
    MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read)(fh, buf, count, datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    ...
    int count, MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_at)(fh, offset, buf,
    ...
    int count, MPI_Datatype datatype, MPI_Status * status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_at_all)(fh, offset, buf,
        count, datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
...
int MPI_File_read_all(MPI_File fh, void * buf, int count, MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_all)(fh, buf, count,
        datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_COLL_READS, tm1, tm2);
...
int MPI_File_read_shared(MPI_File fh, void * buf, int count, MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_shared)(fh, buf, count,
        datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_INDEP_READS, tm1, tm2);
    ...
    MPI_Datatype datatype, MPI_Status * status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_ordered)(fh, buf, count,
        datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_COLL_READS, tm1, tm2);
    ...
    int count, MPI_Datatype datatype)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_at_all_begin)(fh, offset, buf,
        count, datatype);
    tm2 = scrmfs_wtime();
    CP_LOCK();
...
int MPI_File_read_all_begin(MPI_File fh, void * buf, int count, MPI_Datatype datatype)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_all_begin)(fh, buf, count, datatype);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_SPLIT_READS, tm1, tm2);
    CP_UNLOCK();
...
int MPI_File_read_ordered_begin(MPI_File fh, void * buf, int count, MPI_Datatype datatype)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_read_ordered_begin)(fh, buf, count,
        datatype);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_SPLIT_READS, tm1, tm2);
    ...
    int count, MPI_Datatype datatype, __D_MPI_REQUEST *request)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_iread_at)(fh, offset, buf, count,
        datatype, request);
    tm2 = scrmfs_wtime();
    CP_LOCK();
...
int MPI_File_iread(MPI_File fh, void * buf, int count, MPI_Datatype datatype, __D_MPI_REQUEST * request)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_iread)(fh, buf, count, datatype, request);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_NB_READS, tm1, tm2);
    CP_UNLOCK();
    ...
    MPI_Datatype datatype, __D_MPI_REQUEST * request)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_iread_shared)(fh, buf, count,
        datatype, request);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_READ(ret, fh, count, datatype, CP_NB_READS, tm1, tm2);
    ...
    MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write)(fh, buf, count, datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    ...
    int count, MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_at)(fh, offset, buf,
        count, datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_INDEP_WRITES, tm1, tm2);
    ...
    int count, MPI_Datatype datatype, MPI_Status * status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_at_all)(fh, offset, buf,
        count, datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
...
int MPI_File_write_all(MPI_File fh, void * buf, int count, MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_all)(fh, buf, count,
        datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_COLL_WRITES, tm1, tm2);
...
int MPI_File_write_shared(MPI_File fh, void * buf, int count, MPI_Datatype datatype, MPI_Status *status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_shared)(fh, buf, count,
        datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_INDEP_WRITES, tm1, tm2);
    ...
    MPI_Datatype datatype, MPI_Status * status)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_ordered)(fh, buf, count,
         datatype, status);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_COLL_WRITES, tm1, tm2);
    ...
    int count, MPI_Datatype datatype)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_at_all_begin)(fh, offset,
        buf, count, datatype);
    tm2 = scrmfs_wtime();
    CP_LOCK();
...
int MPI_File_write_all_begin(MPI_File fh, void * buf, int count, MPI_Datatype datatype)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_all_begin)(fh, buf, count, datatype);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_SPLIT_WRITES, tm1, tm2);
    CP_UNLOCK();
...
int MPI_File_write_ordered_begin(MPI_File fh, void * buf, int count, MPI_Datatype datatype)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_write_ordered_begin)(fh, buf, count,
        datatype);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_SPLIT_WRITES, tm1, tm2);
    ...
    int count, MPI_Datatype datatype, __D_MPI_REQUEST *request)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_iwrite_at)(fh, offset, buf,
...
int MPI_File_iwrite(MPI_File fh, void * buf, int count, MPI_Datatype datatype, __D_MPI_REQUEST * request)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_iwrite)(fh, buf, count, datatype, request);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_NB_WRITES, tm1, tm2);
    CP_UNLOCK();
    ...
    MPI_Datatype datatype, __D_MPI_REQUEST * request)
{
    ...
    tm1 = scrmfs_wtime();
    ret = SCRMFS_MPI_CALL(PMPI_File_iwrite_shared)(fh, buf, count,
        datatype, request);
    tm2 = scrmfs_wtime();
    CP_LOCK();
    CP_RECORD_MPI_WRITE(ret, fh, count, datatype, CP_NB_WRITES, tm1, tm2);
    ...
    char* logfile_name, MPI_Offset* next_offset)
{
    ...
    MPI_Op reduce_op;
    MPI_Datatype rtype;
    struct scrmfs_file* tmp_array = NULL;
    int shared_count = 0;

    /* register a reduction operation */
    ret = SCRMFS_MPI_CALL(PMPI_Op_create)(scrmfs_file_reduce, 1, &reduce_op); 
    if(ret != 0)
    {
        return(-1);
    }
     ...
     */
    SCRMFS_MPI_CALL(PMPI_Type_contiguous)(sizeof(struct scrmfs_file),
        MPI_BYTE, &rtype); 
    SCRMFS_MPI_CALL(PMPI_Type_commit)(&rtype); 
    ...
    ret = SCRMFS_MPI_CALL(PMPI_Bcast)(hash_array,
        (CP_MAX_FILES * sizeof(uint64_t)), 
        MPI_BYTE, 0, MPI_COMM_WORLD);
    if(ret != 0)
    {
        SCRMFS_MPI_CALL(PMPI_Op_free)(&reduce_op);
        SCRMFS_MPI_CALL(PMPI_Type_free)(&rtype);
        return(-1);
    }
    ...
    ret = SCRMFS_MPI_CALL(PMPI_Allreduce)(mask_array, all_mask_array,
    ...
    if(shared_count)
    {
        if(rank == 0)
        {
            ...
            tmp_array = malloc(shared_count*sizeof(struct scrmfs_file));
            if(!tmp_array)
            {
        ...
        qsort(final_job->file_array, final_job->file_count, 
            sizeof(struct scrmfs_file), file_compare);

        ret = SCRMFS_MPI_CALL(PMPI_Reduce)(
        ...
        if(rank == 0)
        {
            ...
            memcpy(&final_job->file_array[final_job->file_count-shared_count],
                tmp_array, shared_count*sizeof(struct scrmfs_file));
            free(tmp_array);
            tmp_array = NULL;
        }
        else
...
static void scrmfs_file_reduce(void* infile_v, 
    ...
    MPI_Datatype *datatype)
{
    struct scrmfs_file tmp_file;
    struct scrmfs_file* infile = infile_v;
    struct scrmfs_file* inoutfile = inoutfile_v;
    struct scrmfs_file_runtime tmp_runtime;
    int i;
    int j;
    int k;
    ...
    char* logfile_name, int count, int* lengths, void** pointers, double start_log_time)
{
    ...
    if(count > 0)
    {
        ...
        SCRMFS_MPI_CALL(PMPI_Type_hindexed)(count, lengths, displacements,
            MPI_BYTE, &mtype);
        SCRMFS_MPI_CALL(PMPI_Type_commit)(&mtype); 
    }
    ...
    ret = SCRMFS_MPI_CALL(PMPI_File_open)(MPI_COMM_WORLD, logfile_name,
    ...
    if(ret != MPI_SUCCESS)
    {
        if(rank == 0)
        {
            ...
            fprintf(stderr, "scrmfs library warning: unable to open log file %s: %s\n", logfile_name, msg);
        }
        if(count > 0)
            SCRMFS_MPI_CALL(PMPI_Type_free)(&mtype);
        return(-1);
    }
  
    /* figure out where everyone is writing */
    if(count > 0)
        SCRMFS_MPI_CALL(PMPI_Type_size)(mtype, &my_total);
    else
        my_total = 0;
    my_total_long = my_total;
    SCRMFS_MPI_CALL(PMPI_Scan)(&my_total_long, &offset, 1,
    ...
    else
    {
         ...
         * collectivee anyway 
         */
        ret = SCRMFS_MPI_CALL(PMPI_File_write_at_all)(fh, offset, NULL, 
            0, MPI_BYTE, &status);
        if(ret != MPI_SUCCESS)
        {
            failed_write = 1;
    ...
    SCRMFS_MPI_CALL(PMPI_File_close)(&fh);
...
static struct scrmfs_file_runtime* walker_file = NULL;
...
static int file_compare(const void* a, const void* b)
{
    const struct scrmfs_file* f_a = a;
    const struct scrmfs_file* f_b = b;
...
static char* scrmfs_get_exe_and_mounts(struct scrmfs_job_runtime* final_job)
{
    ...
    /* length of exe has already been safety checked in scrmfs-posix.c */
        ...
        if(ret == 0)
        {
            ...
            if(mnt_array_index < CP_MAX_MNTS)
            {
                mnt_hash_array[mnt_array_index] =
                    scrmfs_hash((void*)entry->mnt_dir, strlen(entry->mnt_dir), 0);
    ...
    int count, int rank)
{
    ...
    ret = SCRMFS_MPI_CALL(PMPI_Op_create)(pairwise_variance_reduce, 1,
        &pw_var_op);
    if (ret != MPI_SUCCESS)
    {
        goto error_handler;
    ...
    ret = SCRMFS_MPI_CALL(PMPI_Type_contiguous)(sizeof(struct variance_dt),
        MPI_BYTE, &var_dt);
    if (ret != MPI_SUCCESS)
    {
        goto error_handler;
    ...
    ret = SCRMFS_MPI_CALL(PMPI_Type_commit)(&var_dt);
    if (ret != MPI_SUCCESS)
    {
        goto error_handler;
    }
...
error_handler:
    if (var_dt != MPI_BYTE) SCRMFS_MPI_CALL(PMPI_Type_free)(&var_dt);
    if (pw_var_op != MPI_OP_NULL) SCRMFS_MPI_CALL(PMPI_Op_free)(&pw_var_op);
...
/* record any hints used to write the scrmfs log in the log header */
static void cp_log_record_hints(struct scrmfs_job_runtime* final_job, int rank)
{
    char* hints;
    char* header_hints;
    ...
    meta_remain = SCRMFS_JOB_METADATA_LEN - 
...
static struct scrmfs_file_runtime* scrmfs_file_by_name_setfh(const char* name, MPI_File fh)
{
    ...
    tmp_file = scrmfs_file_by_name_sethandle(name, &fh, sizeof(fh), SCRMFS_FH);
    return(tmp_file);
}

static void scrmfs_file_close_fh(MPI_File fh)
{
    scrmfs_file_closehandle(&fh, sizeof(fh), SCRMFS_FH);
    return;
}
...
static struct scrmfs_file_runtime* scrmfs_file_by_fh(MPI_File fh)
{
    ...
    tmp_file = scrmfs_file_by_handle(&fh, sizeof(fh), SCRMFS_FH);
